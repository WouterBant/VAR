...
class Localization:
    ...
    def triangulate_2d_ls(self, marker_detection_results):
        ...
        distances = np.array([
            ...
            for marker_id, distance in zip(marker_detection_results["marker_ids"], 
    ...
    def triangulate_3d_ls(self, marker_detection_results):
        ...
        result = least_squares(residuals, initial_guess, args=(landmarks, distances), ftol=1e-10, xtol=1e-10, gtol=1e-10)
        ...
        if len(marker_detection_results["marker_ids"]) > 2:
            ...
            for i in range(len(marker_ids)):
                if i not in closest_indices:
                    ...
                    error1 += abs(np.sqrt((p.x - point1[0])**2 + (p.y - point1[1])**2) - measured_dist)
                    error2 += abs(np.sqrt((p.x - point2[0])**2 + (p.y - point2[1])**2) - measured_dist)
