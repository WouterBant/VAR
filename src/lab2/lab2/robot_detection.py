...
class RobotDetection:
    ...
    def detect(self, frame, show_line=True, show_contour_area=True, show_live_detection=True):
        ...
        if show_line:
            image_copy = self.draw_lines(image_copy, percentile_horizontal, percentile_to_keep_vertical)
            mask = self.draw_lines(cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR), percentile_horizontal, percentile_to_keep_vertical)
        ...
        for contour in contours:
            ...
            if contour_area > COUNTOUR_AREA_THRESHOLD:  # Adjust the threshold if needed
                ...
                if vertical_height_bottom_left_corner > BOTTOM_LEFT_OBJECT_HEIGHT_THRESHOLD:
                    # print(f"left bottom corner is at height: {vertical_height_bottom_left_corner}")
                    # print(f"vertical image height in pixels: {height}")
                    if draw_circle_bool:
                        self.draw_circle(image_copy, (x,vertical_height_bottom_left_corner))
                    cv2.rectangle(image_copy, (x, y), (x + w, y + h), (0, 255, 0), 2)

                    # Add left/middle/right
                    loc_description_set = self.determine_location(width, bbox, percentile_to_keep_vertical)
