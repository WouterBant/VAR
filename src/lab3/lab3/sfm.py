...
class SfM:
    ...
    def estimate_camera_motion(self):
        ...
        for i in range(len(self.matches)):
            ...
            img1_kp = np.float32(
                [self.keypoints[i][m.queryIdx].pt for m in self.matches[i]]
            )
            img2_kp = np.float32(
                [self.keypoints[i + 1][m.trainIdx].pt for m in self.matches[i]]
            )

            # Estimate fundamental matrix
            F, mask = cv2.findFundamentalMat(img1_kp, img2_kp, cv2.FM_RANSAC)

            # Recover pose
            _, R, t, _ = cv2.recoverPose(F, img1_kp, img2_kp)
    ...
    def triangulate_points(self, camera_matrices):
        ...
        for i, (R, t) in enumerate(camera_matrices):
            ...
            # Triangulate points
            points_4d = cv2.triangulatePoints(P0, P1, img1_kp.T, img2_kp.T)
    ...
    def run_sfm(self):
        ...
        point_clouds = self.filter_point_cloud(
            point_clouds
        )  # TODO make it work without filtering
