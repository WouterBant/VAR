...
class SfMPipeline:
    ...
    def estimate_fundamental_matrices(self, features: List[Tuple], matches: List[List[cv2.DMatch]]):
        """
    ...
    def triangulate_points(self, P1: np.ndarray, P2: np.ndarray, pts1: np.ndarray, pts2: np.ndarray):
        """
    ...
    def visualize_matches(self, features, matches):
        ...
        for match_set in matches[0]:  # Visualize matches for the first image's neighborhood
            src_idx = match_set['source_idx']
            tgt_idx = match_set['target_idx']
