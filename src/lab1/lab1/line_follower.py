...
class LineFollower:
    ...
    def crop_to_top_roi(self, img, roi_height_ratio=0.5):
        ...
        if self.debug > 3: self.display_image('Cropped Image', cropped_img)
        return cropped_img
    ...
    def binarize_image(self, img):
        ...
        if self.debug > 3: self.display_image('Binary Image', binary)
        return binary
    ...
    def action(self, best_line):
        ...
        if best_line is None:
            ...
            line_angle = (line_angle - 90) / 180 if line_angle > 0 else (90 + line_angle) / 180
